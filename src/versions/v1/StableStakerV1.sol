...
/**
 * @title StableStakerV1
 * @notice FROZEN SOURCE of the deployed V1 {StableStaker}.
 *         Deployed address : 0xbce8ABC09BaEDCabE93419bF875f6186e182079A (Ethereum mainnet)
 *         Source commit    : c3ec65bf0115e9bcc0a75f705a1c8cb57b32ce94
 *         Deploy date      : 2026-06-10 (ResumeStableStakerMigration, phase-2-staging story 055)
 *         Recorded in      : reflax-mint/phase-2-staging/server/deployments/mainnet-addresses.ts
 *
 *         Reproduce with:  git show c3ec65b:src/StableStaker.sol
 *         The ONLY permitted divergences from that output are listed under
 *         "Permitted divergences" below. Everything else, bugs included, is verbatim.
 *
 *         Known defects deliberately preserved: ss14m1 (terminal migration bricked by
 *         setYieldStrategy's unrecorded idle sweep), ss14l8 (set-aside buffer excluded
 *         from the migration realized amount R). Do NOT fix them here.
 *
 *         ## Permitted divergences (story 019)
 *
 *         1. Contract name `StableStaker` -> `StableStakerV1`, on the `contract` declaration
 *            (one line) and on the `@title` of the original NatSpec block immediately below
 *            (one line). Required because two contracts named `StableStaker` in one `src/`
 *            tree make Foundry's artifact resolution ambiguous.
 *         2. This frozen header block, prepended above the original NatSpec block. The
 *            original block is otherwise byte-for-byte as at `c3ec65b`.
 *         3. Import paths / pragma: NONE were required. The file compiles against the
 *            current `lib/` submodule pins exactly as written at `c3ec65b`.
 *
 *         Revert strings reading "StableStaker: ..." are ABI-visible behaviour of the
 *         deployed bytecode and are verbatim. Prose/NatSpec mentions of "StableStaker"
 *         in the body describe the deployed instance and are verbatim. Nothing else —
 *         not formatting, not comments, not unused-variable warnings — was touched.
 *
 *         Verify with:
 *           git show c3ec65b:src/StableStaker.sol | diff - src/versions/v1/StableStakerV1.sol
 *         The diff must contain exactly this header block and the two rename lines.
 *
 ...
 */
contract StableStakerV1 is Ownable, Pausable, ReentrancyGuard, IPausable {
